...
#ifdef CONFIG_ARCH_SYNAPTICS
	char dev_hwpart_str[5];
	int dev = mmc_get_env_dev();
	uint part = mmc_get_env_part((struct mmc*)str);

	snprintf(dev_hwpart_str, sizeof(dev_hwpart_str), "%d.%d", dev, part);
	ret = blk_get_device_by_str("mmc", dev_hwpart_str, &desc);
#else
...
static inline s64 mmc_offset(int copy)
{
	...
	err = mmc_offset_try_partition(copy ? ENV_MMC_PART_NAME_REDUND : ENV_MMC_PART_NAME, &val);
#else
	err = mmc_offset_try_partition(ENV_MMC_PART_NAME, &val);
#endif
	if (!err)
		return val;
#endif
...
#if defined(CONFIG_SYS_MMC_ENV_PART) || defined(CONFIG_ARCH_SYNAPTICS)
__weak uint mmc_get_env_part(struct mmc *mmc)
{
#ifdef CONFIG_ARCH_SYNAPTICS
	return get_mmc_part_by_name(mmc_get_env_dev(), (const char*)mmc);
#else
