...
static bool fxl6408_gpio_is_output(struct udevice *dev, unsigned off)
{
	u8 val;
...
static int fxl6408_gpio_get_function(struct udevice *dev, unsigned off)
{
	if (fxl6408_gpio_is_output(dev, off))
		return GPIOF_OUTPUT;
...
static int fxl6408_gpio_get_value(struct udevice *dev, unsigned off)
{
	...
	reg = fxl6408_gpio_is_output(dev, off) ? FXL6408_OUTPUT : FXL6408_INPUT_STATUS;

	val = fxl6408_i2c_read_le8(dev, reg);
	if (val < 0)
		return val;
	return (val & BIT(off)) != 0;
}
...
static int fxl6408_gpio_probe(struct udevice  *dev)
{
	...
	if (tmp_str)
		snprintf(bank_name, sizeof(bank_name), "%s@%x_", tmp_str, addr);
	else
		snprintf(bank_name, sizeof(bank_name), "gpio@%x_", addr);
