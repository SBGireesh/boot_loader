...
static int syna_spi_child_pre_probe(struct udevice *dev)
{
	...
	slave->max_read_size = plat->max_read_size;
...
static int syna_spi_ofdata_to_platdata(struct udevice *bus)
{
	...
	ret = dev_read_u32(bus, "max-read-size", &mxrsz);
	if (ret) {
		debug("Set default max read size %d\n", SPI_RX_MAX_DATA_LEN - MAX_SPI_CMD_LEN);
		plat->max_read_size = SPI_RX_MAX_DATA_LEN - MAX_SPI_CMD_LEN;
	} else {
		plat->max_read_size = mxrsz;
	}
